#ifndef METAL_LIST_NTH_ELEMENT_VAL_RANDPIVOT_HPP
#define METAL_LIST_NTH_ELEMENT_VAL_RANDPIVOT_HPP
...
namespace metal {
namespace nth_element_val_randpivot {
    namespace detail {
        template<typename seq>
        using _quickselect_pivot =
            at<seq, mod<number<4678930447522229821>, size<seq>>>;
        ...
        struct _quickselect<lo, eq, hi, num, false_, true_> {
            using num_in_eq = metal::sub<num, size<lo>>;

            template<template<typename...> class comp>
            using type = at<eq, num_in_eq>;
        };
