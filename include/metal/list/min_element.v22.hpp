#ifndef METAL_LIST_MIN_ELEMENT_V22_HPP
#define METAL_LIST_MIN_ELEMENT_V22_HPP
...
namespace v22 {
    ...
    namespace detail {
        ...
        struct _min_element_min {
            template<class x, class y>
            using type =
              metal::detail::forward<_min_element_min_impl<x, y>::template type, expr>;
        };
        ...
        struct _min_element<lambda<expr>> {
            template<class seq>
            using type = metal::detail::forward<_min_element_impl<seq>::template type, expr>;
        };
    }
    /// \endcond
}
